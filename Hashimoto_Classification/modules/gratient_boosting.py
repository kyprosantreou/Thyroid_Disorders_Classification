...
def gradient_boosting_classification():
    ...
    gb = GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, max_depth=3, random_state=30)    
    gb.fit(X_train, y_train)

    # Predictions
    y_pred = gb.predict(X_test)
    y_prob = gb.predict_proba(X_test)[:, 0]  # Probability for class 0

    # Encode categorical labels
    y_test_numeric = label_encoder.fit_transform(y_test)
    y_test_inverted = 1 - y_test_numeric  # Make 'hypothyroid' = 1 and 'negative' = 0
